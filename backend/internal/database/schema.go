...
// initSQL contains the full database schema and seed data.
// It is inlined here so the binary is self-contained and works in any
// deployment environment (Railway, Docker, etc.) without needing access
// to the scripts/ directory at runtime.
//
// The statements use IF NOT EXISTS / ON CONFLICT DO NOTHING, so this
// function is safe to call on every startup — it is a no-op when the
...
// InitializeSchema creates all tables, indexes, triggers, and seed data
// defined in the embedded SQL schema. It is idempotent: every DDL statement
// uses IF NOT EXISTS and every INSERT uses ON CONFLICT DO NOTHING, so calling
// this on an already-initialized database is safe and produces no side effects.
