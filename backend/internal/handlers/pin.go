...
func (h *PinHandler) SetPin(c *gin.Context) {
	...
	if role != "manager" && role != "admin" {
		c.JSON(http.StatusBadRequest, models.APIResponse{Success: false, Message: "PINs can only be set for managers or admins"})
		return
	}
...
func (h *PinHandler) VerifyPin(c *gin.Context) {
	...
	var userName string
	err := h.db.QueryRow(`SELECT first_name || ' ' || last_name FROM users WHERE manager_pin = $1 AND role IN ('manager','admin') AND is_active = true LIMIT 1`, req.Pin).Scan(&userName)
	if err != nil {
